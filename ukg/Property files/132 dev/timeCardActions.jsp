...
function checkAmountInputs(){
	const amountCollection = document.getElementsByClassName("amounts");
	var isCorrectInput = true;
	for(var i=0; i < amountCollection.length && isCorrectInput; i++){
		console.log("Checking Amount :",amountCollection[i].value);
		if(!ifCorrectInput(amountCollection[i],false)){
			isCorrectInput=false;
			document.getElementById("U_I_10").value="false";
			console.log("Amount Error Found : ",amountCollection[i].value);
			amountCollection[i].focus();
			return false;
		}
	}
	return true;
...
function confirmation(){
	...
	var ifCorrectInputs = checkAmountInputs();
	if(ifCorrectInputs){
...
<html:hidden property="value(isInputCorrect)" styleId="U_I_10" />
					...
					<c:forEach var="item" items="${webServiceConfigTable}" varStatus="status">
						<c:set var="itemId" value="${item.getPersonNum()}" />
							...
							<tr class="last-child <c:if test='${rowIndex % 2 !=  0}'>Even</c:if>">
								...
								<td class="last-child" style="text-align:center;" >
									<input name="value(${item.getPersonNum()}_amount)" value="${item.getAmount()}" size="3" maxlength="6" autocomplete="off" class="amounts"
